...
namespace KeyFinder{
  ...
  void Parameters::setSegGaussianSigma(float sigma){
    if(!std::isfinite(sigma)) throw Exception("Gaussian sigma cannot be NaN");
  ...
  void Parameters::setDirectSkStretch(float stretch){
    if(!std::isfinite(stretch)) throw Exception("Spectral kernel stretch cannot be NaN");
  ...
  void Parameters::setDetunedBandWeight(float weight){
    if(!std::isfinite(weight)) throw Exception("Detuned band weighting cannot be NaN");
